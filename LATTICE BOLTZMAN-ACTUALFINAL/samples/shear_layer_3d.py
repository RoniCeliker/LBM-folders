...
def main():
    ...
    velocity1 = [Ma * stencil.cs, 0.0, 0.0]
    velocity2 = [0.5 * Ma * stencil.cs, 0.0, 0.0]
    velocity = [0.75 * Ma * stencil.cs, 0.0, 0.0]
    density = 1.0
    ...
    ## Create mesh
    mesh = create_mesh(height=height, depth=depth, plate_length=plate_length)
    # # Write mesh to file
...
def create_mesh(height: int, depth: int, plate_length: int):
    mesh_gen = MeshGenerator()

    # East
    boundary_names = {"west": None, "east": "OUTLET", "south": "BOT", "north": "TOP",
                      "back": "WALL", "front": "WALL"}
    mesh_gen.create_block(nx=7*plate_length, ny=2*height, nz=depth, x_offset=plate_length, y_offset=-height,
                          boundary_names=boundary_names, renumber=False)
    # North west
    boundary_names = {"west": "INLET_TOP", "east": None, "south": "PLATE_TOP", "north": "TOP",
                      "back": "WALL", "front": "WALL"}
    mesh_gen.create_block(nx=plate_length, ny=height, nz=depth, x_offset=0, y_offset=0,
                          boundary_names=boundary_names, renumber=False)
    # South west
    boundary_names = {"west": "INLET_BOT", "east": None, "south": "BOT", "north": "PLATE_BOT",
                      "back": "WALL", "front": "WALL"}
    mesh_gen.create_block(nx=plate_length, ny=height, nz=depth, x_offset=0, y_offset=-height,
                          boundary_names=boundary_names, renumber=False)
